...
module.hot.dispose(reloadCSS);
module.hot.accept(reloadCSS);
},{"_css_loader":"../../node_modules/.pnpm/parcel-bundler@1.12.5/node_modules/parcel-bundler/src/builtins/css-loader.js"}],"../code/Card.jsx":[function(require,module,exports) {
...
},{}],"../static_files/d3.js":[function(require,module,exports) {
var define;
...
},{}],"../code/d3_demo.jsx":[function(require,module,exports) {
var d3 = require("../static_files/d3"); // create the container that gets exported
...
var radius = d3.scale.sqrt().range([0, 7]);
var scale = d3.scale.linear();
var svg = d3Container.append("svg").attr("width", width).attr("height", height);
var svg2 = d3Container.append("svg").attr("width", width).attr("height", height);
var force = d3.layout.force().size([width, height]).charge(-400).linkStrength(3).linkDistance(function (d) {
...
var node = svg.selectAll(".node").data(graph.nodes).enter().append("g").attr("class", function (d) {
  ...
  // if(isConnected(d, o)) {
  d3.select(this).select("circle").style("stroke-width", 6); // Figure out the neighboring node id's with brute strength because the graph is small
  ...
  svg.selectAll("circle").style("stroke", "gray"); // now we select the neighboring circles and apply whatever style we want.
  ...
  d3.select(this).select("circle").style("stroke-width", 1.5); //d3.select(this).select("circle").style("stroke", "gray");
...
var xScale = d3.scale.linear().domain([d3.min(graph.locs, function (d) {
  return d.long;
}), d3.max(graph.locs, function (d) {
  return d.long;
})]).range([padding, width - 4 * padding]);
var yScale = d3.scale.linear().domain([d3.min(graph.locs, function (d) {
  return d.lat;
}), d3.max(graph.locs, function (d) {
  return d.lat;
})]).range([height - padding, padding]);
var mapit = svg2.selectAll(".mapit").data(graph.locs).enter().append("g").on("mouseover", function (d) {
...
mapit.append("rect").attr("x", function (d) {
  return xScale(d.long);
}).attr("y", function (d) {
  return yScale(d.lat);
}).attr("height", 20).attr("width", 20).attr("fill", "gray").attr("stroke-width", "1.5").attr("stroke", "gray").attr("class", function (d) {
  return d.location;
});
mapit.append("text").attr("x", function (d) {
  return xScale(d.long);
}).attr("y", function (d) {
  return yScale(d.lat) - 5;
}).attr("font-family", "sans-serif").attr("font-size", "12px").attr("fill", "black").text(function (d) {
  return d.location;
...
function tabulate(tdata, columns) {
  var table = d3Container.append("table"),
      thead = table.append("thead"),
      tbody = table.append("tbody");
  thead.append("tr").selectAll("th").data(columns).enter().append("th").text(function (column) {
    return column;
  });
  var rows = tbody.selectAll("tr").data(tdata).enter().append("tr").attr("class", function (d) {
...
var subdata = graph.nodes.filter(function (d) {
  return d.location != "x";
});
...
var Card = require("./code/Card");

var d3Demo = require("./code/d3_demo"); // create the body
...
}, "Hello World!"), /*#__PURE__*/React.createElement(Card, {
  title: "This is a D3 Demo",
  style: {
    width: "50rem"
  }
}, d3Demo));
