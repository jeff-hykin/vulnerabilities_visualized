...
const commitLogsFolder = path.join(process.env.FORNIX_FOLDER, "data", "commit_logs")
for (const whichFileName of fs.readdirSync(commitLogsFolder)) {
    if (whichFileName.match(/.txt$/g)) {
        const whichFile = path.join(commitLogsFolder, whichFileName)
